def run_loop(game_running):
    while True:
        if game_running == "PlayTopia":
            user_input = input(
                "Enter 'stay' to stay in "
                + game_running
                + " and pick another game, or 'quit' to leave: "
            )
            if user_input.lower() == "stay":
                print("Returning to the " + game_running + " menu...")
                return "restart"  # Signal to restart
            elif user_input.lower() == "quit":
                print("Exiting " + game_running + "...")
                return "quit"  # Signal to quit
            else:
                print(f"You entered: {user_input}")
        else:
            user_input = input(
                "Enter 'stay' to keep playing " + game_running + ", 'quit' to exit: "
            )
            if user_input.lower() == "stay":
                print("Restarting " + game_running + "...")
                return "restart"  # Signal to restart
            elif user_input.lower() == "quit":
                print("Exiting " + game_running + "...")
                return "quit"  # Signal to quit
...
print(
    """ 
        ...
        We have a range of games to offer, including:
...
while True:
    ...
    if game == code_for_GTN:
        ...
        while True:
            ...
            time.sleep(
                4
            )  # pausing between the message display and the beggining of the game.
            ...
            # asking whether to replay or quit
            action = run_loop("Guess the Number")
            if action == "quit":
                break  # exit game
            elif action == "restart":
                continue  # replay this game
            ...
            while not finished:
                ...
                if user_choice == comp_choice:
                    ...
                    if comp_choice == "scissors":
                        finished = True
                        print(
                            f"You lose. You chose {user_choice} and the computer chose {comp_choice}. You have {consecutive_wins} consecutive wins."
                        )
                    else:
                        print(
                    ...
                    if comp_choice == "rock":
                        ...
                        consecutive_wins += win_point
                action = run_loop("Paper scissors rock")
                if action == "quit":
                    break
                elif action == "restart":
                    continue
